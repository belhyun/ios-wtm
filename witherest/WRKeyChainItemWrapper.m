...
#import "WRKeychainItemWrapper.h"

@implementation WRKeychainItemWrapper
...
+ (BOOL)compareKeychainValueForMatchingPIN:(NSUInteger)pinHash
{
    ...
    if ([[self keychainStringFromMatchingIdentifier:@"hasSavedPIN"] isEqualToString:[self securedSHA256DigestHashForPIN:pinHash]]) {
        return YES;
    } else {
        return NO;
    }
...
+ (NSString *)securedSHA256DigestHashForPIN:(NSUInteger)pinHash
{
    ...
    // 2
    NSString *computedHashString = [NSString stringWithFormat:@"%@%i%@", name, pinHash, @"AAaaBBbbCCccDDddEEeeFFffGGggHHhhIIii1234567890HD2GrfxaZ84oGA8RsKdNRpxdAojXYg9iAj"];
    // 3
